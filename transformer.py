...
def _generate_square_subsequent_mask(
    ...
    """
    #torch.triu() is a function in PyTorch that returns the upper triangular part of a tensor
    return torch.triu(
        torch.full((sz, sz), float("-inf"), dtype=dtype, device=device),
        diagonal=1,
    )
    #This creates a causal mask for attention:
    #float("-inf") blocks attention.
    #diagonal=1 means everything above the main diagonal is masked.
    #Used to prevent a token from attending to future tokens.
...
class Transformer(Module):
    ...
    def _reset_parameters(self):
        ...
        #Your model inherits from nn.Module, which provides the parameters() method.
        for p in self.parameters():
            if p.dim() > 1:
                xavier_uniform_(p)
                #xavier_uniform_ is a weight initialization function from PyTorch's torch.nn.init module.
                #We initialize tensors like weights using methods like xavier_uniform_ to ensure stable and effective learning from the very first forward and backward pass.
...
class TransformerEncoder(Module):
    ...
    def forward(
        self,
        src: Tensor,
        #the mask argument can be a Tensor or None
...
class TransformerEncoderLayer(Module):
    ...
    def forward(
        ...
        #is a performance-related switch introduced in PyTorch to control whether a faster implementation of Multi-Head Attention (MHA) is enabled.
        is_fastpath_enabled = torch.backends.mha.get_fastpath_enabled()
...
class TransformerDecoderLayer(Module):
    ...
    # here is cross-attention mechanism. 
    def _mha_block(
        self,
        x: Tensor,
        mem: Tensor,
